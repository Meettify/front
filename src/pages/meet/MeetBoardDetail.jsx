...
const MeetBoardDetail = () => {
    const { id: meetBoardId } = useParams();
    ...
    useEffect(() => {
        const fetchDetail = async () => {
            try {
                ...
                await fetchPostDetail(meetBoardId);
                await fetchComments(meetBoardId);
            } catch (error) {
                console.error('게시물 조회 실패:', error);
            }
        };

        if (meetBoardId && isFirstLoad.current) {
            fetchDetail();
    ...
    const handleCommentSubmit = async () => {
        ...
        try {
            await addComment(meetBoardId, commentContent);
            setCommentContent('');
        } catch (error) {
    ...
    const confirmEdit = async () => {
        try {
            await updateComment(meetBoardId, editingCommentId, editingContent);
    ...
    const handleDeleteComment = async (commentId) => {
        try {
            await deleteComment(meetBoardId, commentId);
            await fetchComments(meetBoardId);
        } catch (error) {
    ...
    const submitReply = async () => {
        ...
        try {
            await addComment(meetBoardId, replyContent, replyingCommentId);
            setReplyingCommentId(null);
            setReplyContent('');
            await fetchComments(meetBoardId);
        } catch (error) {
        ...
        <div className="text-left max-w-2xl mx-auto p-4">
            ...
            {isAuthor && (
                <div className="flex justify-end space-x-3 pb-5">
                    <RoundedCancelButton onClick={() => navigate(`/meetBoards/${meetBoardId}`)}>
                        수정
                    </RoundedCancelButton>
                    <RoundedDeleteButton
                        onClick={async () => {
                            await deletePost(meetBoardId);
