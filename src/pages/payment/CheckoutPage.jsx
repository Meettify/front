...
export function CheckoutPage({ user, price, orderItems, disabled, orderId, onFail, onSuccess }) {
  ...
  const handlePaymentSuccess = () => {
    ...
    const orderNames =
      orderItems.length > 1
        ? `${orderItems[0]?.item?.itemName || '상품명 없음'} 외 ${orderItems.length - 1}건`
        : orderItems[0]?.item?.itemName || "상품명 없음"; // itemName을 제대로 참조

    console.log("Processing payment for order:", { orderId, orderNames, customerName: user?.memberName, customerEmail: user?.memberEmail });
    ...
    paymentWidget
      .requestPayment({
        ...
        customerEmail: user?.memberEmail || "customer@example.com",
      ...
      .then((response) => {
        ...
        navigate(`/success?orderId=${orderId}&paymentKey=${response.paymentKey}&amount=${price}`);
        if (onSuccess) onSuccess(); // onSuccess 호출
      })
      .catch((error) => {
        navigate(`/fail??code=${error}&message=${error_message}&orderId=${orderId}`);
