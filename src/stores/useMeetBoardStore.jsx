...
const useMeetBoardStore = create((set) => ({
  ...
  setMeetBoardPermission: (permission) =>
    set({ meetBoardPermission: permission }),
  setLoading: (loading) => set({ loading }),

  fetchPosts: async (page = 0, size = 1, sortDirection = "desc", meetId) => {
    set({ loading: true, posts: [] });
    try {
      console.log(
        `Fetching posts - Page: ${page}, Size: ${size}, Sort: postDate,${sortDirection}, MeetId: ${meetId}`
      );

      // MeetBoardList 호출하여 데이터를 받아옴
      const response = await MeetBoardList(
        meetId,
        page,
        size,
        `postDate,${sortDirection}`
      );
  ...
  fetchPostDetail: async (meetBoardId) => {
    ...
    try {
      ...
      const { meetBoardDetailsDTO, meetBoardPermission, comments } =
        await getMeetBoardDetail(meetBoardId);
  ...
  updatePost: async (
    ...
    set({ loading: true });
    try {
      await updateMeetBoard(meetBoardId, title, content, remainImgId, newFiles);
      set({ loading: false });
    } catch (error) {
