...
const EditProfileForm = () => {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "updateMemberPw") {
      ...
      if (value !== "" && formData.originalMemberPw === value) {
        setValidationErrors((prev) => ({
          ...prev,
          updateMemberPw: validation.isValid
            ? "기존 비밀번호는 사용할 수 없습니다."
            : "",
        }));
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await putUpdateMember(
    ...
    <div className="w-1/2 ml-40">
      <form onSubmit={handleSubmit} className="form-container">
        ...
        {validationErrors.updateMemberPw && (
          <p className="error-message">{validationErrors.updateMemberPw}</p>
        )}
        ...
        {passwordFocused && (
          <div className="password-popup top-[390px]">
            ...
            <ul>
              ...
              <li className={passwordValid.hasAlphabet ? "valid" : ""}>
                알파벳 포함
              </li>
              <li className={passwordValid.hasNumber ? "valid" : ""}>
                숫자 포함
              </li>
              <li className={passwordValid.hasSpecialChar ? "valid" : ""}>
                특수 문자 포함
              </li>
            </ul>
          </div>
        ...
        <button
          type="submit"
          className={`p-2 mt-5 w-[288px] rounded text-white ${
            isButtonDisabled ? "bg-blue-500" : "bg-blue-300"
          }`}
