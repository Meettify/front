...
const SectionAddr = React.memo(() => {
    ...
    useEffect(()=>{
        ...
        const getUser = async () => {
            ...
            try {
                ...
                console.error("회원 조회 실패 : ", err.response ? err.response.data : err);
            }
        };
    ...
    return(
        <>
            ...
            <div className="form-group">
                <div className="input-group">
                    <textarea className="form-control" name="" id="" placeholder=" "></textarea>
                    <label htmlFor="">배송메모</label>
                </div>
            </div>
            {modalAddr?<ModalAddr addr={addr} setModalAddr={setModalAddr} setAddr={setAddr}/>:""}
        </>
    )
});
